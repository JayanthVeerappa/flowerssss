...
for word in flowerChoice.split():
    if word.isdigit():
        numberof = int(word)
    if word.lower() == 'lily' or word.lower() == "lilys":
    ...
    if word.lower() == 'sunflower' or word.lower() == "sunflowers":
        for i in range (1, numberof):
            sunflower()
            painter.penup()
            painter.goto(float(screen[0])*((i+1)/numberof),0)
            painter.pendown()
            painter.setheading(0)
    if word.lower() == 'rose' or word.lower() == "roses":
        for i in range(1,numberof):
            rose()
            painter.penup()
            painter.goto(float(screen[0])*((i+1)/numberof),0)
            painter.pendown()
            painter.setheading(0)
    if word.lower() == 'daises' or word.lower() == "diasy":
        print ("YE SIT WOKR i love u U DHRUV")
    if word.lower() == 'tulips' or word.lower() == "tulips":
        print ("YE SIT WOKR i love u U DHRUV")
trtl.done()
